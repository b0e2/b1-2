...
Supabase 를 고른 이유는 다루는 데이터가 한 종류이고, **값의 범위와 조건을 테이블 제약으로 그대로 표현할 수 있기** 때문입니다. 
...
로그인 화면과 Not Found 만 보호 바깥에 있습니다. 사이드바의 `NavLink` 다섯 개(홈·학습 목록·태그·통계·설정)가 주요 화면을 잇고, 우상단 사용자 버튼이 설정으로 갑니다.
